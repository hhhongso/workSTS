...
//강사님 	
	public void makeSearchPagingHTML() {
		...
		if(startPage > pageBlock) pagingHTML.append("[ <span id='paging' class='"+(startPage-1)+"')'> 이전 </span> ]");		
		for(int i = startPage; i <= endPage ; i++) {
			if(i == currentPage) pagingHTML.append("[ <span id='currPaging' class='"+i+"'>"+ i +"</span> ]"); //onclick=location.href='../board/boardSearch?pg="+ i +"'
			else pagingHTML.append("[ <span id = 'paging' class='"+i+"'>"+ i +"</span> ]");			
		}
		
		if(endPage < totalPage) pagingHTML.append("[ <span id='paging' class='"+ (endPage+1) +"')'> 다음 </span> ]");
